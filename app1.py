    ...
    for original_item in MARKET_DATA_CACHE:
        ...
        if coin in active_portfolio:
            ...
            if "ENGINE LOCKED" not in item["fase"]:
                if item["fase"] in ["INSTITUTIONAL BUY", "VALID BREAKOUT", "EARLY RALLY", "EARLY RALLY (WEAK VOL)", "⚡ SQUEEZE BREAKOUT (EARLY TREND)", "🐳 WHALE ACCUMULATION (SILENT)", "🔄 MOMENTUM REVERSAL (BOTTOMING)"]:
