...
async def check_bitcoin_circuit_breaker(client):
    ...
    try:
        ...
        res_1h, res_5m = await asyncio.gather(client.get(url, timeout=4), client.get(url_5m, timeout=4))
...
def calculate_ma(prices, period):
    if len(prices) < period: return sum(prices) / len(prices) if prices else 0.0
...
def calculate_obv_trend(klines_1h):
    if len(klines_1h) < 10: return True
    obv = 0
    obv_values = []
...
def calculate_atr_and_spread(klines_1d, klines_1h):
    ...
    if avg_recent_spread == 0: avg_recent_spread = 0.00001
...
async def process_single_coin_pipeline(client, symbol, m_data, user_portfolio, semaphore, device_id="default_guest_device"):
    ...
    async with semaphore:
        ...
        klines_1w, klines_1d, klines_1h, klines_15m, order_book_ratio = await asyncio.gather(task_1w, task_1d, task_1h, task_15m, task_depth)
        if not klines_1w or not klines_1d or not klines_1h or not klines_15m or len(klines_1h) < 40 or len(klines_1d) < 99: return None
